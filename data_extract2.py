...
d_dir  = cwd + '/Exp2_new/' # get the path to the data directory
f_name = os.listdir(d_dir)     # get all the data file names
f_name.sort();

all_f  = open('AC2_new.csv', 'w') # open a file to save cleaned data

header = ['subj', 'name', 'gender', 'age', 'shape', 'dist', 'td_dif', 'resp', 'resp_key', 'd_loc', 't_loc',
			'gotKey','startTime','respTime','resp', 'fix_loc', 'runNum', 'prac']
        ...
        for line in d_f:
            ...
            if msg[0] == 'conf':
                ...
                if msg[0] != 'subj':                    
                    ...
                    if msg[9] == str(tar_locs[new_id]):
                        msg[9] = 'fd'
                    if msg[9] == str(tar_locs[new_id+1]) or msg[9] == str(tar_locs[new_id-1]):
                        msg[9] = 'fd1'
                    if msg[9] == str(tar_locs[new_id+2]) or msg[9] == str(tar_locs[new_id-2]):
                        msg[9] = 'fd2'
                    if msg[9] == str(tar_locs[new_id+3]) or msg[9] == str(tar_locs[new_id-3]):
                        msg[9] = 'fd3'
                    if msg[9] == str(tar_locs[new_id-4]):
                        msg[9] = 'fd4'
